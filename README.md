# ИИ-юрист— бот для разбора договоров

Telegram-бот для первичного анализа договоров в формате **.doc/.docx**. Бот принимает документ, строит **схему в стиле BPMN** и предоставляет пользователю основные действия через кнопки: повторно показать схему, вывести краткое описание договора, показать ключевые риски и загрузить новый договор. Такой формат помогает быстрее понять логику процесса, состав участников, последовательность шагов и потенциально проблемные места в договоре.
...
- Кнопка **«Показать BPMN»** — повторно открывает актуальную схему процесса
- Кнопка **«Кратко о договоре»** — выводит краткое описание договора и его основных блоков
- Кнопка **«Ключевые риски»** — показывает потенциально проблемные места и возможные направления улучшений
- Кнопка **«Уточнить / внести правку»** — позволяет текстом добавить или скорректировать шаги процесса, после чего схема обновляется
- Кнопка **«Загрузить новый договор»** — запускает новый сценарий анализа для другого документа
